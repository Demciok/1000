...
def test_set_hand_1():
    # 6 kart w kolorze kier (cały dostępny zestaw) + 1 karo na szum
    return [
...
def test_set_hand_3():
    # Wszystkie cztery 9 z talii (w sumie 0 pkt) + najniższe figury jako tło
    return [
...
def test_set_hand_4():
    # Cztery Asy (Kareta) + dwie Damy (Para) + dwie 10
    return [
        Card("kier", "A", 11, "czerwo"),
        Card("pik", "A", 11, "wino"),
...
def test_turn_must_follow_suit():
    return Turn(
        1,
        {
...
def test_turn_trumped_by_opponent():
    return Turn(
        2,
        {
...
def test_turn_opponent_discards_trash():
    return Turn(
        3,
        {
